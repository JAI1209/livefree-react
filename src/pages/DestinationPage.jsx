...
const NAVBAR_H = 80 // navbar actual height (logo 66px + padding ~28px)

// ── Full-bleed helper — same trick as Hero ──
const fullBleed = {
  width: '100vw',
  position: 'relative',
  left: '50%',
...
function GallerySection({ allPhotos, onOpenLightbox }) {
  ...
  paddingBottom: 8,
      ...
      {/* inner container re-constrains content */}
...
<div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 8, height: 420, position: 'relative' }}>
  <div
    style={{ borderRadius: '12px 0 0 12px', overflow: 'hidden', cursor: 'pointer' }}
    onClick={() => onOpenLightbox(allPhotos, 0)}
  >
    ...
    >
      SEE ALL PHOTOS 📸
          ...
          <div style={{
            ...
            <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'rgba(232,93,58,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: '#a07060', fontSize: '0.92rem', lineHeight: 1.6 }}>Images for this property will be available shortly.</p>
            </div>
          </div>
        )}
      </div>
...
function DirectionAccordion({ label, content }) {
  ...
  return (
    <div style={{ borderRadius: 10, border: '1px solid #eee', marginBottom: 10, background: '#fff', overflow: 'hidden' }}>
      <button
        onClick={() => setOpen(v => !v)}
        style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', background: 'none', border: 'none', cursor: 'pointer', fontWeight: open ? 700 : 500, fontSize: '0.92rem', color: open ? 'var(--primary)' : 'var(--text)', textAlign: 'left' }}
      >
        {label}
        <ChevronDown size={16} style={{ transform: open ? 'rotate(180deg)' : 'none', transition: '0.2s', color: 'var(--primary)', flexShrink: 0 }} />
      </button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.22 }} style={{ overflow: 'hidden' }}>
            <p style={{ padding: '0 18px 16px', fontSize: '0.88rem', color: '#666', lineHeight: 1.7 }}>{content}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function AmenitiesSection() {
  ...
  const toggleAmenity = (id) => setAmenities(prev => prev.map(a => a.id === id ? { ...a, checked: !a.checked } : a))

  return (
    <div style={{ ...fullBleed, background: '#fff', borderTop: '1px solid #eee', borderBottom: '1px solid #eee', padding: '24px 0' }}>
      <div style={{ maxWidth: 'var(--container)', margin: '0 auto', padding: '0 28px' }}>
        <h3 style={{ fontWeight: 800, fontSize: '1rem', letterSpacing: '3px', textTransform: 'uppercase', marginBottom: 16, color: 'var(--primary)' }}>
          AMENITIES
        </h3>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          {checkedItems.map(a => {
            const Icon = a.icon
            return (
              <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.85rem', color: 'var(--text)', fontWeight: 500, border: '1px solid #eee', borderRadius: 8, padding: '8px 14px', background: '#fff' }}>
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.85rem', color: '#999', fontWeight: 500, border: '1px dashed #ddd', borderRadius: 8, padding: '8px 14px', background: 'transparent', cursor: 'pointer', transition: 'all 0.15s' }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--primary)'; e.currentTarget.style.color = 'var(--primary)' }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = '#ddd'; e.currentTarget.style.color = '#999' }}
          >
            <Plus size={15} style={{ color: 'var(--primary)' }} /> Add amenity
          </button>
        </div>
        <AnimatePresence>
          {expandOpen && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25, ease: [0.2, 0.9, 0.4, 1] }} style={{ overflow: 'hidden' }}>
              <div style={{ marginTop: 16, border: '1px solid #eee', borderRadius: 12, background: '#fff', overflow: 'hidden' }}>
                {categories.map((cat, ci) => {
                  ...
                  return (
                    <div key={cat} style={{ borderBottom: ci < categories.length - 1 ? '1px solid #f5f5f5' : 'none', padding: '16px 20px' }}>
                      <p style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#aaa', marginBottom: 10 }}>{cat}</p>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '4px 12px' }}>
                        {catItems.map(a => {
                          const Icon = a.icon
                          return (
                            <div key={a.id} onClick={() => toggleAmenity(a.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0', fontSize: '0.84rem', cursor: 'pointer', color: a.checked ? 'var(--primary)' : 'var(--text)', fontWeight: a.checked ? 600 : 400, transition: 'color 0.15s', userSelect: 'none' }}>
                              <span style={{ width: 18, height: 18, borderRadius: '50%', border: `1.5px solid ${a.checked ? 'var(--primary)' : '#ddd'}`, background: a.checked ? 'var(--primary)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.15s' }}>
                ...
                <div style={{ padding: '12px 20px', borderTop: '1px solid #f5f5f5', display: 'flex', justifyContent: 'flex-end' }}>
                  <button onClick={() => setExpandOpen(false)} style={{ padding: '8px 22px', borderRadius: 8, fontSize: '0.83rem', fontWeight: 600, background: 'var(--primary)', color: '#fff', border: 'none', cursor: 'pointer' }}>Done</button>
                </div>
              </div>
            </motion.div>
          )}
  ...
  return (
    <section style={{ padding: '48px 0', background: '#fff', borderBottom: '1px solid #eee' }}>
      <div className="container">
        <h2 style={{ fontWeight: 800, fontSize: '1.6rem', textAlign: 'center', marginBottom: 24, letterSpacing: 1 }}>Characteristics</h2>
          ...
          {topPills.map(label => {
            ...
            return (
              <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', border: '1px solid #ddd', borderRadius: 8, fontSize: '0.85rem', fontWeight: 500, color: 'var(--text)', background: '#fff' }}>
                <Icon size={16} style={{ color: '#555' }} /> {label}
              </div>
            )
          })}
        </div>
            ...
            return (
              <div key={cat.id}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                  ...
                  <span style={{ fontWeight: 800, fontSize: '0.78rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#222' }}>{cat.label}</span>
                </div>
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {cat.items.map(item => (
                    <li key={item} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.87rem', color: '#555' }}>
                      <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#bbb', flexShrink: 0 }} /> {item}
                    </li>
                  ))}
                </ul>
              </div>
    ...
    <section style={{ padding: '48px 0', background: '#fafafa' }}>
      <div className="container">
        <div style={{ border: '1px solid #e8e8e8', borderRadius: 14, background: '#fff', padding: '32px 36px' }}>
          <h2 style={{ fontWeight: 800, fontSize: '0.9rem', letterSpacing: '3px', textTransform: 'uppercase', textAlign: 'center', marginBottom: 28, color: '#222' }}>Important Information</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'start' }}>
            <div style={{ borderRight: '1px solid #f0f0f0', paddingRight: 32 }}>
              <div style={{ marginBottom: 16 }}><span style={{ fontWeight: 700, fontSize: '0.92rem', color: '#222' }}>Entrance: </span><span style={{ fontSize: '0.92rem', color: '#555' }}>{info.checkIn}</span></div>
              <div style={{ marginBottom: 20 }}><span style={{ fontWeight: 700, fontSize: '0.92rem', color: '#222' }}>Exit: </span><span style={{ fontSize: '0.92rem', color: '#555' }}>{info.checkOut}</span></div>
              {info.extraBeds && (<div><p style={{ fontWeight: 700, fontSize: '0.92rem', color: '#222', marginBottom: 6 }}>Extra beds</p><p style={{ fontSize: '0.88rem', color: '#666', lineHeight: 1.6 }}>{info.extraBeds}</p></div>)}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {info.rules.map((rule, i) => (<p key={i} style={{ fontSize: '0.88rem', color: '#666', lineHeight: 1.65, margin: 0 }}>{rule}</p>))}
            </div>
          </div>
        </div>
      </div>
...
<div style={{
  ...
  <div className="container">
    ...
    <h1 style={{
      ...
      LiveFree Hostel <span style={{ color: 'var(--primary)', fontStyle: 'italic' }}>{dest.name}</span>
    </h1>
    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', marginTop: 20 }}>
      {meta.highlights.slice(0, 4).map(h => (
        <span key={h} style={{
          ...
          borderRadius: 99, background: 'rgba(232,93,58,0.1)',
      ...
      {/* ── Date Picker Bar — full bleed, sticky below navbar ── */}
<div style={{
  ...
  boxShadow: '0 2px 12px rgba(0,0,0,0.06)',
        ...
        <div style={{ maxWidth: 'var(--container)', margin: '0 auto', padding: '0 28px', display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button className="btn btn-primary" style={{ padding: '10px 28px', fontWeight: 700, fontSize: '0.9rem' }}>CHECK RATES</button>
        </div>
      </div>
      ...
      <section style={{ padding: '56px 0', background: '#fff' }}>
        <div className="container">
          <h2 style={{ fontWeight: 800, fontSize: '1rem', letterSpacing: '3px', textTransform: 'uppercase', marginBottom: 32, color: 'var(--primary)' }}>ROOM TYPES</h2>
              ...
              return (
                <div key={room.id} style={{ borderBottom: '1px solid #f0f0f0', padding: '24px 0' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr auto', gap: 24, alignItems: 'start' }}>
                    <div style={{ borderRadius: 10, overflow: 'hidden', aspectRatio: '4/3', background: '#eee', cursor: heroPhoto ? 'pointer' : 'default' }} onClick={() => heroPhoto && openLightbox(roomPhotos, 0)}>
                      {heroPhoto
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {room.amenities.map(a => (<span key={a} style={{ fontSize: '0.72rem', padding: '3px 10px', borderRadius: 99, background: '#f5f5f5', color: '#666', fontWeight: 500 }}>{a}</span>))}
                      </div>
                      {roomPhotos.length > 1 && (
                        <button onClick={() => openLightbox(roomPhotos, 0)} style={{ marginTop: 10, fontSize: '0.78rem', color: 'var(--primary)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: 0 }}>View {roomPhotos.length} photos →</button>
                      )}
                      <button onClick={() => setExpandedRoom(isExpanded ? null : room.id)} style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.78rem', fontWeight: 700, color: isExpanded ? 'var(--primary)' : 'var(--text)', background: 'none', border: 'none', cursor: 'pointer', letterSpacing: '0.08em', textTransform: 'uppercase', padding: 0, transition: 'color 0.2s' }}>
                    ...
                    <div style={{ textAlign: 'right', minWidth: 140 }}>
                      ...
                      <a href={room.bookingUrl} target="_blank" rel="noreferrer" className="btn btn-primary" style={{ fontSize: '0.82rem', padding: '8px 20px', whiteSpace: 'nowrap' }}>Book a Room</a>
                    </div>
                  </div>
                  <AnimatePresence>
                    {isExpanded && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.28, ease: [0.2, 0.9, 0.4, 1] }} style={{ overflow: 'hidden' }}>
                        <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px dashed #eee', paddingLeft: 224 }}>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px 20px' }}>
                            {room.amenities.map(a => { const Icon = getAmenityIcon(a); return (<div key={a} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.84rem', color: '#444', fontWeight: 500 }}><Icon size={15} style={{ color: '#888' }} />{a}</div>) })}
                          </div>
                        </div>
                      </motion.div>
                    )}
          ...
          {rooms.length > 3 && (
            <div style={{ textAlign: 'center', marginTop: 24 }}>
              <button onClick={() => setShowAllRooms(!showAllRooms)} className="btn btn-ghost" style={{ fontSize: '0.88rem' }}>{showAllRooms ? 'Show less' : 'SHOW MORE +'}</button>
            </div>
          )}
        </div>
      </section>
          ...
          <div className="container">
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 24 }}>
              {Object.keys(photosByCategory).map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)} style={{ padding: '6px 16px', borderRadius: 99, fontSize: '0.82rem', fontWeight: 600, border: '1.5px solid', borderColor: activeCategory === cat ? 'var(--primary)' : '#ddd', background: activeCategory === cat ? 'var(--primary)' : '#fff', color: activeCategory === cat ? '#fff' : 'var(--text)', cursor: 'pointer', transition: '0.2s' }}>{cat}</button>
              ))}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
              {activePhotos.map((id, i) => (
                <motion.div key={id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} onClick={() => openLightbox(activePhotos, i)} style={{ borderRadius: 10, overflow: 'hidden', aspectRatio: '4/3', cursor: 'pointer', background: '#eee' }}>
                  <img src={cld(id)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.3s' }} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.05)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'} />
                </motion.div>
              ))}
            </div>
          </div>
      ...
      <section style={{ background: '#fff', borderTop: '1px solid #eee' }}>
        <div className="container" style={{ padding: '0 28px' }}>
          <button onClick={() => setCancelOpen(!cancelOpen)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '22px 0', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
            <span style={{ fontWeight: 700, fontSize: '1rem' }}>Cancellation Policy — <span style={{ color: 'var(--text-muted)', fontWeight: 400, fontStyle: 'italic' }}>click to know more</span></span>
      ...
      <section style={{ padding: '56px 0', background: '#fdf6f0' }}>
        <div className="container" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'start' }}>
          <div>
            <span style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: 'var(--primary)' }}>DIRECTIONS</span>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.4rem, 2.5vw, 2rem)', fontWeight: 700, margin: '10px 0 28px', lineHeight: 1.2 }}>Reach us without the guesswork.</h2>
            {Object.entries(meta.directions).map(([label, content]) => (<DirectionAccordion key={label} label={label} content={content} />))}
          </div>
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {meta.highlights.map(h => (<div key={h} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.88rem', color: 'var(--text-muted)' }}><span style={{ color: 'var(--primary)' }}>📍</span> {h}</div>))}
            </div>
          </div>
        </div>
      </section>
      ...
      {itinerary.length > 0 && (
        <section style={{ padding: '56px 0', background: '#fdf6f0' }}>
          <div className="container" style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: 56, alignItems: 'start' }}>
            <div>
              <img src="https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=500&q=80" alt="Travel" style={{ width: '100%', borderRadius: 16, boxShadow: 'var(--shadow-md)' }} />
            </div>
            <div>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '0.92rem', lineHeight: 1.75, marginBottom: 28 }}>Wake up to fresh mornings, friendly faces, and an energy that is unique to the cities we call home.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {itinerary.map((day, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }} style={{ background: '#fff', borderRadius: 12, padding: '20px 24px', boxShadow: 'var(--shadow-sm)', borderLeft: '3px solid var(--primary)' }}>
                    <h4 style={{ fontWeight: 700, fontSize: '0.95rem', marginBottom: 12, color: 'var(--text)' }}>{day.day}</h4>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {day.activities.map((a, j) => (<div key={j} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.88rem', color: 'var(--text-muted)' }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--primary)', flexShrink: 0 }} />{a}</div>))}
                    </div>
                  </motion.div>
                ))}
              </div>
              <div style={{ marginTop: 28 }}><a href="/contact" className="btn btn-primary">know more</a></div>
            </div>
          </div>
        </section>
      )}
      ...
      <AnimatePresence>
        {lightboxIndex !== null && (
          <LightBox images={lightboxPhotos} index={lightboxIndex} onClose={closeLightbox}
