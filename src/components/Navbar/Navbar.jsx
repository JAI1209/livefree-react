...
const ABOUT_ITEMS = [
  ...
  { label: 'Pet Friendly', sub: 'Welcoming your companions', icon: <Heart size={15} />, path: '/pet-friendly' },
]
const PROPERTIES_ITEMS = [
  { label: 'Rishikesh', sub: 'Dev Bhoomi · Land of Gods', icon: <MapPin size={15} />, path: '/rishikesh' },
  { label: 'Dehradun', sub: 'Gateway to Himalayas', icon: <MapPin size={15} />, path: '/dehradun' },
  { label: 'Varanasi', sub: 'City of Light', icon: <MapPin size={15} />, path: '/varanasi' },
]
        ...
        <div key={item.path}>
          ...
          <button className={styles.dropdownItem} onClick={() => onSelect(item.path)}>
            ...
            <div className={styles.dropdownItemText}>
              ...
              {item.sub && <div className={styles.dropdownItemSub}>{item.sub}</div>}
            </div>
          </button>
        </div>
      ))}
    ...
    <>
      ...
      <header className={`${styles.header} ${scrolled ? styles.scrolled : ''}`} ref={navRef}>
        <div className={`container ${styles.topbar}`}>
            ...
            <ul className={styles.navList}>
              ...
              <li className={styles.navItem}>
                <button
                  ...
                  onClick={() => { goTo('/'); setTimeout(() => document.getElementById('experiences')?.scrollIntoView({ behavior: 'smooth' }), 100) }}
                >
                  Experiences
                </button>
              </li>
          ...
          <button className={`${styles.ctaBtn} btn btn-primary`} onClick={() => goTo('/')}>
            Book Your Stay
          </button>
      ...
      <nav className={`${styles.mobileNav} ${mobileOpen ? styles.open : ''}`} aria-label="Mobile navigation">
        ...
        {/* About accordion */}
        <button
          className={`${styles.mobileNavLink} ${mobileExpanded === 'about' ? styles.open : ''}`}
          ...
          {mobileExpanded === 'about' && (
            ...
            >
              {ABOUT_ITEMS.map(item => (
                <button key={item.path} className={styles.mobileSubItem} onClick={() => goTo(item.path)}>
        ...
        <button
          className={`${styles.mobileNavLink} ${mobileExpanded === 'properties' ? styles.open : ''}`}
